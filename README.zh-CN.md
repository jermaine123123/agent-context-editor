...
它为受支持的 Agent 宿主提供一种一致、可恢复且由用户掌控的上下文管理方式，作为提示词引导和自动上下文压缩的补充，而不是替代。

## 使用建议与注意事项

长时间运行的 Agent 会话可能出现 **上下文腐化（context rot）**：随着上下文不断增长，尤其当失败尝试、过时结论和无关 Tool 输出逐渐累积时，即使上下文窗口尚未满，模型检索并正确使用关键信息的可靠性也可能下降。

对于大多数任务，使用提示词引导 Agent 和依赖自动上下文压缩通常已经足够。手动排除和编辑上下文是自动结果不符合预期时的可选控制手段，而不是用户需要持续进行的操作。当自动压缩遗漏了希望保留的细节，或者失败尝试和无关历史占据了大量上下文时，可以用它更精确地调整后续发送给模型的内容。

> **提示缓存影响：** 排除或编辑上下文会改变发送给模型的输入前缀，使最早改动位置之后的提示缓存可能无法继续复用。改动越靠近 Session 末尾，通常受影响的缓存越少；越靠近开头，影响通常越大。具体行为取决于 Agent 宿主、模型和服务商的缓存机制。原始 Session 仍会保留。

## 当前功能

- 手动排除选中的上下文，并随时恢复
- 手动编辑 User 消息和 AI Answer，支持恢复原文与撤销
- 使用 AI 精简选中的上下文，预览后替换并可随时恢复（选择性压缩）
- 搜索对话并在多个结果之间跳转
...
手动排除、文字编辑和选中上下文 AI 精简目前支持 Pi TUI 与 DeepSeek Harness。可编辑内容仅限纯文本 User 消息和完整、无签名的 Answer。
...
输入 `/ctx` 打开全屏 Context Editor。主要操作包括使用 `x` 排除或恢复上下文、`e` 编辑文字、`E` 恢复原文、`h` 和 `r` 进行视觉隐藏与恢复，以及使用 `s` 切换搜索范围。使用 `c` 生成并预览选中上下文的摘要，`C` 排除或恢复已应用的摘要，`D` 恢复精简前的原文。按 `?` 可以查看完整快捷键说明。
...
视觉修改和模型上下文修改分别保存。视觉隐藏只改变 Context Editor。经过确认的上下文排除、受支持的文字编辑和已应用的精简摘要只改变后续发送给模型的派生输入，不覆盖原始 Session，也不修改宿主的主聊天时间线。
