...
整篇对话、按 User、AI、Tool 类型筛选消息、选择单条或多条记录，以及隐藏、恢复和
撤销视图修改。隐藏状态单独保存，原始 Session 不会被删除或改写。

项目的长期目标是在不同 Agent 宿主中提供一致的对话编辑方式：精简长会话，重新打开
会话时快速找到用户选择保留的信息，控制哪些内容参与后续模型上下文，并让 AI 精简
选中的片段或整篇会话。
