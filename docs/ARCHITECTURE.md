...
The Pi extension keeps its established V1 state and native Tool Output
projection. The Pi Context Desktop fork uses the same V2 record/unit contract
but remains a separate Electron repository so upstream history and binaries do
not become part of the adapter package.
