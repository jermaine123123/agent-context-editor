...
describe('DeepSeek Harness installable bundle', () => {
  ...
  it('keeps apply and inject on the module namespace for the Cordis loader', () => {
    ...
    expect(hostEntry).toContain('contextExclusion: true')
  ...
  it('exports the direct Remote surface with context replacement methods', () => {
    expect(descriptors.map(descriptor => descriptor.method)).toEqual([
      'getSnapshot', 'listRecords', 'getRecord', 'searchRecords', 'getSearchMatch', 'previewContext', 'previewReplacement', 'commitContext', 'commitView', 'undoView', 'commitReplacement', 'restoreReplacement', 'undoReplacement',
    ])
    ...
    expect(TYPERT.invocations).toHaveLength(13)
  })
})
