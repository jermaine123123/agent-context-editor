    ...
    function ContextEditorView({ sessionId, controller, useSession }) {
    	...
    	const restoreCondensation = async (operationId) => {
    		...
    		try {
    			...
    			if (result?.restoreRequired) {
    				setCondensationError(result.restoreMode === "checkpoint" ? text.condensationCheckpoint(result.checkpointSeq) : text.condensationRestoreUnavailable);
    				await refresh(true);
    				return;
    			}
    			if (result?.conflict) {
    	...
    	const renderCondensation = (item) => {
    		...
    		return h("article", {
    			...
    			type: "button",
    			disabled: readOnly || restoreBlocked,
    			onClick: () => void restoreCondensation(item.operationId)
    		}, text.restoreCondensation)), h("pre", { className: "context-editor__condensation-summary" }, item.summary), h("div", {
